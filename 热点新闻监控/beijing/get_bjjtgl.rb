...
if (doc/"div#wuhang>ul>li a").to_s == ""
    res = Net::HTTP.get_response(URI.parse("#{ARGV[2]}"))
    doc = Hpricot(res.body)
end
    

(doc/"div#wuhang>ul>li").each do |news|
    href = "http://www.bjjtgl.gov.cn" + news.search("/a")[0].attributes['href'].strip
    title = "[beijing]" + Iconv.iconv("GBK//IGNORE", "UTF-8", news.search("/a").inner_html.to_s)[0].strip
