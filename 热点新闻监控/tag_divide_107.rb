...
for name in dir
    ...
    mapfile = "./output/107/#{name}/#{suffix}"
    if File.exist?("#{mapfile}")
        file = File.open("#{mapfile}", "r")
        while line = file.gets
            line = line.chomp
            command_arr = line.split
            command = "ruby " + command_arr[0] + " " + command_arr[1] + dir_num["#{name}"] + command_arr[2] + " output/107/output/#{day} " + command_arr[3] + " 2>>error_107.log"
            res = system("#{command}")
            if (!res)
                print "false #{command}\n"
            end
...
if (flag)
    res = system("cat #{t1}.html | mutt -e 'set content_type=\"text/html\"' -e 'set charset=\"gbk\"' -e 'my_hdr from:WangRuishan@baidu.com' -s \"hot news\" \"wangruishan@baidu.com\"")
    if (!res)
